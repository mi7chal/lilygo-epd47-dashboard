...
void DisplayManager::showCustomScreen(const CustomScreenData& data) {
    ...
    displayWrappedString(InterBold24, data.title, epd_width() / 2, epd_height() / 2 - titleBlockHeight - 24 , contentWidth, Alignment::Center);
    displayWrappedString(InterRegular16, data.message, epd_width() / 2, epd_height() / 2, contentWidth, Alignment::Center);
...
void DisplayManager::goToSleep(uint32_t sleepDurationSeconds, uint32_t delayBeforeSleepMs) {
    ...
    logger::info("sleep in {} ms for {} s", static_cast<unsigned int>(delayBeforeSleepMs), static_cast<unsigned int>(sleepDurationSeconds));
...
void DisplayManager::displayString(const GFXfont& font, const char* str, int32_t x, int32_t y, Alignment horizontalAlignment) {
...
void DisplayManager::displayString(const GFXfont& font, const std::string& str, int32_t x, int32_t y, Alignment horizontalAlignment) {
    displayString(font, str.c_str(), x, y, horizontalAlignment);
}

void DisplayManager::displayWrappedString(const GFXfont& font, const std::string& str, int32_t x, int32_t y, int32_t maxWidth, Alignment horizontalAlignment, int32_t lineSpacing) {
